...
def color_to_str(c):
    """Format an RGB color (or grayscale) for Mitsuba XML.

    Accepts a 3-tuple/list/ndarray, a single number (grayscale), or a
    preformatted string. Returns a string like 'r, g, b' or None.
    """
    if c is None:
        return None
    if isinstance(c, str):
    ...
    try:
        if hasattr(c, '__len__') and len(c) == 3:
            return f"{float(c[0])}, {float(c[1])}, {float(c[2])}"
        # grayscale numeric
...
def add_quad(scene, name, origin, target, up, scale=0.1, bsdf_is_checker=False, albedo=None):
    ...
    if bsdf_is_checker:
        ...
        # support albedo as a callable or a direct color
        if callable(albedo):
            try:
                a = albedo(origin)
            except TypeError:
                a = albedo(origin, target, up)
        else:
            a = albedo
        sval = color_to_str(a) or '0.8'
        SubElement(bsdf, 'rgb', name='reflectance', value=sval)


def add_quad_as_cube(scene, name, origin, target, up, scale=0.1, bsdf_is_checker=False, albedo=None):
...
def get_quad_soup_no_group(scene, bbox_min=[-1, -1, -1], bbox_max=[1, 1, 1], n=10, scale=0.1, bsdf_checker=False, albedo_fn=None):
    ...
    for i in range(n):
        ...
        up = [0, 0, 1]
        add_quad_as_cube(scene, f'quad_{i:05d}', origin, target, up, scale=scale,
                         bsdf_is_checker=bsdf_checker, albedo=albedo_fn)
    return scene
...
def set_up_scene(integrator='additionalvertex', simple_geo_level=1, simple_cam=True,
                 ...
                 light_mode=0, albedo_fn=None, return_xml_tree=False):
    ...
    if n_quads is not None and n_quads > 0:
        scene = get_quad_soup_no_group(scene, n=n_quads, scale=0.05, bbox_min=[-1, -1, 0], bbox_max=[1, 1, 3], bsdf_checker=bsdf_checker, albedo_fn=albedo_fn)
    else:
        if simple_geo_level == 1:
            scene = get_quad_soup_no_group(scene, n=1000, scale=0.03, bbox_min=[-1, -1, 0], bbox_max=[1, 1, 3], bsdf_checker=bsdf_checker, albedo_fn=albedo_fn)
        elif simple_geo_level == 2:
            scene = get_quad_soup_no_group(scene, n=20, scale=0.1, bbox_min=[-1, -1, 2], bbox_max=[1, 1, 3], bsdf_checker=bsdf_checker, albedo_fn=albedo_fn)
...
if __name__ == '__main__':
    ...
    for n_quads in [50, 100, 300]:
        for integrator in ['path', 'bdpt', 'doublestep']:
            write_scene(path=f'scene_quads{n_quads}_integrator{integrator}.xml',
                        ...
                        n_quads=n_quads,
                        light_mode=3,
                        albedo_fn=lambda pos: (pos[0] + 1) / 2,  # simple gradient albedo based on x position
                    )