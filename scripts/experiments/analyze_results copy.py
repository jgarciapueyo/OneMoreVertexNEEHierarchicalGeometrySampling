...
def _center_crop_to_aspect(img: np.ndarray, aspect_x: float, aspect_y: float):
    h, w = img.shape[:2]
    if h <= 0 or w <= 0:
        return img
...
def load_img(path: Path):
    ...
    if EXR_ASPECT_CROP is not None and str(path).lower().endswith(".exr"):
        img = _center_crop_to_aspect(img, EXR_ASPECT_CROP[0], EXR_ASPECT_CROP[1])
...
def export_summary_grid(configs, output_root, inset_bbox, mode="spp", target=8, exposure_scale=1.0, output_dir=None, image_ext="png", output_name=None):
    ...
    for c in configs:
        ...
        valid = []
        for p in runs_dir.glob("spp_*.meta.json"):
    ...
    # Horizontal layout for LaTeX: top row = full images, bottom row = zoomed insets.
    fig_width = max(8.0, 3.0 * len(picks))
    fig, axes = plt.subplots(2, len(picks), figsize=(fig_width, 4.8), gridspec_kw={"wspace": 0.03, "hspace": 0.03})
    if len(picks) == 1:
    ...
    for i, (name, pick) in enumerate(picks):
        ...
        ax_main.set_title(f"{name}\nSPP={pick['spp']} | {pick['time']:.2f}s", fontsize=14, pad=4)
...
def main():
    ...
    if EXR_ASPECT_CROP is not None:
        ax, ay = EXR_ASPECT_CROP
        print(f"Aspect-ratio pre-crop enabled for EXRs: aspect_x={ax:g}, aspect_y={ay:g}")
