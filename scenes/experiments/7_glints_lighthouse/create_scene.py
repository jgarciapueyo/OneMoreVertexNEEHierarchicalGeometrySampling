...
Coordinate convention: y-up.
  Camera at (15, 0, 5) looks toward (0, 0, 5) with up (0, 0, 1).
...
QUAD_OBJ = "quad_2tri.obj"
SNOWFLAKE_OBJ = "snowflake.obj"
...
def add_integrator(scene, integrator_type: str):
    ...
    elif integrator_type == "glinttracermis":
        integ = SubElement(scene, "integrator", type="glinttracermis")
        SubElement(integ, "integer", name="granularity", value="200000")
        SubElement(integ, "integer", name="maxDepth", value="2")
        SubElement(integ, "boolean", name="hideEmitters", value="false")  # no direct sampling
...
def add_sensor(scene, resx: int, resy: int, samples: int, fov: float = 45.0):
    ...
    SubElement(t, "lookAt", origin="5, 0, 2", target="0, 0, 3", up="0, 0, 1")
...
def add_spotlight(scene,
    ...
    """
    s = SubElement(scene, "shape", type="sphere", name="light_sphere")
    SubElement(s, "float", name="radius", value="0.1")
    b = SubElement(s, "bsdf", type="diffuse")
    SubElement(b, "rgb", name="reflectance", value="0.9, 0.0, 0.0")
    t = SubElement(s, "transform", name="toWorld")
    SubElement(t, "translate", x=f"{_f(origin[0])}", y=f"{_f(origin[1]+0.1)}", z=f"{_f(origin[2])}")
    """
...
def add_lighthouse(scene):
    """Add a large sphere and a lighthouse OBJ to the scene, to provide some
    non-glinty geometry for visual reference."""
    shape = SubElement(scene, "shape", type="obj", name="lighthouse")
    SubElement(shape, "string", name="filename", value="lighthouse.obj")
    SubElement(shape, "bsdf", type="dielectric", name="transparent")
    t = SubElement(shape, "transform", name="toWorld")
    SubElement(t, "scale", value="0.15")
    SubElement(t, "rotate", x="1", y="0", z="0", angle="90")
    SubElement(t, "translate", x="0", y="1", z="1")
...
def create_scene(
    ...
    vol_min:          Vec3  = (-5.0, -6.0, 0.0),
    vol_max:          Vec3  = (3.0, 5.0, 8.0),
    spot_origin:      Vec3  = (0.0,  0.0,  7.0),
    ...
    add_spotlight(scene,
                origin=(-0.25,  1,  3.7),
                target=(-1,  1.5,  3.7),
                intensity=spot_intensity*0.05,
                cutoff_angle=90,
                beam_width=90)
    """
    # Add constant emitter
    emitter = SubElement(scene, "emitter", type="envmap", name="env")
    SubElement(emitter, "string", name="filename", value="env.exr")
    SubElement(emitter, "float", name="scale", value="0.1")
    t = SubElement(emitter, "transform", name="toWorld")
    SubElement(t, "rotate", x="1", y="0", z="0", angle="70")
    SubElement(t, "rotate", x="0", y="0", z="1", angle="90")
    emitter = SubElement(scene, "emitter", type="constant", name="env")
    SubElement(emitter, "rgb", name="radiance", value="0.01, 0.01, 0.1")
    """

    add_lighthouse(scene)
    ...
    """
    shape = SubElement(scene, "shape", type="obj", name="floor")
    SubElement(shape, "string", name="filename", value=QUAD_OBJ)
    t = SubElement(shape, "transform", name="toWorld")
    SubElement(t, "scale", x=_f(20), y=_f(20), z="1")
    SubElement(t, "rotate", x="0", y="1", z="0", angle="90")
    SubElement(t, "translate", x="-10", y="0", z="0")
    bsdf = SubElement(shape, "bsdf", type="diffuse")
    SubElement(bsdf, "rgb", name="reflectance",
               value=f"{0.85}, {0.5}, {0.5}")
    """

    # point = SubElement(scene, "emitter", type="point", name="point")
    # SubElement(point, "rgb", name="intensity", value="1000, 1000, 1000")
    # t = SubElement(point, "transform", name="toWorld")
    # SubElement(t, "translate", x="0", y="0", z="4")
...
def main():
    ...
    parser.add_argument("--integrator", default="glinttracer",
                        choices=["glinttracer", "glinttracermis", "path", "ptracer", "doublestep"],
                        help="Integrator to embed in the scene XML.")
    parser.add_argument("--n_snowflakes", type=int, default=50000,
                        help="Number of snowflake quads.")
    parser.add_argument("--snowflake_size", type=float, default=0.001,
                        help="Half-size of each snowflake quad (world units).")
    parser.add_argument("--base_color", nargs=3, type=float,
                        default=[0.9, 0.9, 0.9], metavar=("R", "G", "B"),
    ...
    parser.add_argument("--normal_kappa", type=float, default=1.0,
    ...
    parser.add_argument("--spot_intensity", type=float, default=100000.0,
    ...
    parser.add_argument("--resx", type=int, default=700)
    ...
    parser.add_argument("--obj_path", default=QUAD_OBJ,
