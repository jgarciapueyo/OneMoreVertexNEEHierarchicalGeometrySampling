...
Behavior:
  For each matching scene file named like scene_quads{N}_integrator{NAME}.xml,
  the script iterates the allowed spp values and runs:
    mitsuba -S {spp} scene_quads{N}_integrator{NAME}.xml -o scene_quads{N}_integrator{NAME}_spp{SPP}.exr
EOF
}
...
if [[ -n "$INTEGRATOR" ]]; then
  pattern="scene_quads*_integrator${INTEGRATOR}.xml"
else
...
for f in "${files[@]}"; do
  ...
  for spp in "${allowed_spps[@]}"; do
    ...
    out="scene_quads${nquads}_integrator${integratorName}_spp${spp}.exr"
