...
def add_quad(scene, name, origin, target, up, scale=0.1, bsdf_is_checker=False, alpha=0.5):
    ...
    if bsdf_is_checker:
        ...
        SubElement(bsdf, 'float', name='alpha', value=str(alpha))
        SubElement(bsdf, 'rgb', name='reflectance', value='0.8')


def add_quad_as_cube(scene, name, origin, target, up, scale=0.1, bsdf_is_checker=False, alpha=0.5):
...
def get_quad_soup_no_group(scene, bbox_min=[-1, -1, -1], bbox_max=[1, 1, 1], n=10, scale=0.1, bsdf_checker=False, alpha=0.5):
    ...
    for i in range(n):
        ...
        up = [0, 0, 1]
        add_quad_as_cube(scene, f'quad_{i:05d}', origin, target, up, scale=scale, bsdf_is_checker=bsdf_checker, alpha=alpha)
    return scene
...
def set_up_scene(integrator='additionalvertex', simple_geo_level=1, simple_cam=True,
                 ...
                 light_mode=0, alpha=0.5, return_xml_tree=False):
    ...
    if n_quads is not None and n_quads > 0:
        scene = get_quad_soup_no_group(scene, n=n_quads, scale=0.05, bbox_min=[-1, -1, 0], bbox_max=[1, 1, 3], bsdf_checker=bsdf_checker, alpha=alpha)
    else:
        if simple_geo_level == 1:
            scene = get_quad_soup_no_group(scene, n=1000, scale=0.03, bbox_min=[-1, -1, 0], bbox_max=[1, 1, 3], bsdf_checker=bsdf_checker, alpha=alpha)
        elif simple_geo_level == 2:
            scene = get_quad_soup_no_group(scene, n=20, scale=0.1, bbox_min=[-1, -1, 2], bbox_max=[1, 1, 3], bsdf_checker=bsdf_checker, alpha=alpha)
    ...
    for n_quads in [1, 50, 100, 300, 500, 1000]:
        for integrator in ['path', 'bdpt', 'additional_vertex', 'doublestep', 'guided_path']:
            for alpha in [0.001, 0.01, 0.1, 0.5]:
                write_scene(path=f'scene_quads{n_quads}_integrator{integrator}_alpha{alpha}.xml',
