...
class ProjectedAreaLight : public Emitter {
public:
    ProjectedAreaLight(const Properties &props) : Emitter(props) {
        m_type |= EOnSurface | EProjectedAreaEmitter; // our new flag to indicate that this emitter needs to be intersected
    ...
    ProjectedAreaLight(Stream *stream, InstanceManager *manager)
    ...
    Spectrum sampleDirection(DirectionSamplingRecord &dRec,
        ...
        // m_shape->sampleDirection(dRec, pRec, sample); 
        // Vector wi;
        // Float pdf;
        // Point3 selectedPoint;
        // Normal n;

        // AABBSphSample(m_aabb, pRec.p, sample.x, sample.y, wi, pdf, selectedPoint, n);
        ...
        // // Trace a ray from the sampled position along the sampled direction
        // Ray ray(pRec.p, dRec.d, Epsilon, std::numeric_limits<Float>::infinity(), pRec.time);

        // // Check whether the ray intersects the parent shape
        // if (m_shape && m_shape->rayIntersect(ray, ray.mint, ray.maxt, its.t, m_itsbuffer))
        //     return m_radiance;
        // else {
        //     dRec.pdf = 0.0f;
        //     return Spectrum(0.0f);
    ...
    Spectrum sampleDirect(DirectSamplingRecord &dRec,
        ...
        AABBSphSample(m_aabb, dRec.ref, sample.x, sample.y, wi, pdf, p, n);
        ...
        // Now, intersect the shape to find the corresponding point
        // Ray ray(dRec.ref, wi, Epsilon, std::numeric_limits<Float>::infinity(), dRec.time);
        // Intersection its;
        // if (!m_shape->rayIntersect(ray, ray.mint, ray.maxt, its.t, m_itsbuffer)) {
        //     dRec.pdf = 0.0f;
        //     std::cout << "missed shape in area projected light!" << std::endl;
        //     return Spectrum(0.0f);
        // } // cannot do this, since mitsuba1 doesn't have shape->rayIntersect for many triangle-based shapes
        // m_shape->fillIntersectionRecord(ray, m_itsbuffer, its);
        
        // dRec.p = its.p;
        // dRec.n = its.shFrame.n;
        // dRec.dist = (dRec.p - dRec.ref).length();
        // only fill in the fields we know:
        dRec.d = wi;
        dRec.pdf = pdf;
        dRec.measure = ESolidAngle;
        dRec.object = this;
    ...
    void setParent(ConfigurableObject *parent) {
        ...
        if (parent->getClass()->derivesFrom(MTS_CLASS(Shape))) {
            ...
            if (m_shape != NULL)
                Log(EError, "A projected area light cannot be parent of multiple shapes");

            m_shape = shape;
            /* Ensure AABB / surface area are available for sampling */
            m_shape->configure();
            m_aabb = m_shape->getAABB();
            m_surfaceArea = m_shape->getSurfaceArea();
            m_power = m_radiance * M_PI * m_surfaceArea;
        } else {
            Log(EError, "A projected area light must be child of a shape instance");
        }
    }

    AABB getAABB() const {
        return m_shape ? m_shape->getAABB() : AABB();
    }

    std::string toString() const {
        std::ostringstream oss;
        oss << "ProjectedAreaLight[" << endl
...
protected:
    ...
    AABB m_aabb;
    Float m_surfaceArea;
    mutable uint8_t m_itsbuffer[INTERSECTION_TEMP];
};
...
Shader *ProjectedAreaLight::createShader(Renderer *renderer) const {
    return new AreaLightShader(renderer, m_radiance);
}

MTS_IMPLEMENT_CLASS(AreaLightShader, false, Shader)
MTS_IMPLEMENT_CLASS_S(ProjectedAreaLight, false, Emitter)
MTS_EXPORT_PLUGIN(ProjectedAreaLight, "Projected area light");
MTS_NAMESPACE_END
