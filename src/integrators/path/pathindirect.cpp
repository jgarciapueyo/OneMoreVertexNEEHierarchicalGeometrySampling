...
static StatsCounter avgPathLength("Path tracer indirect", "Average path length", EAverage);
 ...
 * }
 */
class MIPathTracerIndirect : public MonteCarloIntegrator {
public:
    MIPathTracerIndirect(const Properties &props)
        : MonteCarloIntegrator(props) { }

    /// Unserialize from a binary data stream
    MIPathTracerIndirect(Stream *stream, InstanceManager *manager)
    ...
    Spectrum Li(const RayDifferential &r, RadianceQueryRecord &rRec) const {
        ...
        while (rRec.depth <= m_maxDepth || m_maxDepth < 0) {
            ...
            if (its.isEmitter() && (rRec.type & RadianceQueryRecord::EEmittedRadiance)
                && (!m_hideEmitters || scattered) && (rRec.depth > 1))
            ...
            /* If a luminaire was hit, estimate the local illumination and
               weight using the power heuristic */
            if (hitEmitter &&
                (rRec.type & RadianceQueryRecord::EDirectSurfaceRadiance) && (rRec.depth > 1)) {
...
MTS_IMPLEMENT_CLASS_S(MIPathTracerIndirect, false, MonteCarloIntegrator)
MTS_EXPORT_PLUGIN(MIPathTracerIndirect, "MI path tracer indirect");
MTS_NAMESPACE_END
