...
#ifndef DEBUG_BVH_NAN
// #define DEBUG_BVH_NAN
#endif

#ifdef DEBUG_BVH_NAN
static inline bool impNonFinite(Float v) {
    return !std::isfinite((double) v);
}

static inline bool impNonFiniteF(float v) {
    return !std::isfinite((double) v);
}

static inline void impLogNodeContext(
    const char *stage,
    ...
    std::cerr << "[DEBUG_BVH_NAN] importance context"
              << " stage=" << stage
              << " nodeIndex=" << nodeIndex
              << " xs=" << its_xs.p.toString()
              << " xe=" << pRec_xe.p.toString();

    if (bvh && nodeIndex >= 0 && (size_t) nodeIndex < bvh->getNodeCount()) {
        const BVHNodeInfo &nodeInfo = bvh->getNodeInfo(nodeIndex);
        Gaussian3D g = nodeInfo.getGaussian3D();
        Float covTrace = g.m_covariance(0,0) + g.m_covariance(1,1) + g.m_covariance(2,2);
        std::cerr << " node.surfaceArea=" << nodeInfo.surfaceArea
                  << " node.valid=" << nodeInfo.valid
                  << " node.mean=" << Point(g.m_mean).toString()
                  << " node.covTrace=" << covTrace
                  << " node.vmfKappa=" << nodeInfo.getVMFKappa()
                  << " node.vmfMu=" << nodeInfo.getVMFMeanDirection().toString()
                  << " node.meanDiffuseLum=" << nodeInfo.getMeanDiffuseAlbedoLum()
                  << " node.gaussianSATreshold=" << bvh->getGaussianSAThreshold();
    }

    std::cerr << std::endl;
}

static inline void impLogValue(
    const char *stage,
    const char *name,
    Float value,
    const GeometryBVH *bvh,
    ...
    std::cerr << "[DEBUG_BVH_NAN] non-finite importance term"
              << " stage=" << stage
              << " name=" << name
              << " value=" << value
              << std::endl;
    impLogNodeContext(stage, bvh, nodeIndex, its_xs, pRec_xe);
}

static inline void impLogValueF(
    const char *stage,
    const char *name,
    float value,
    const GeometryBVH *bvh,
...
#define NODE_IMPORTANCE_VERSION 2
// 0-> before 21/01/26: no aabb solid angle or sggx cross-section
// 1-> after  21/01/26: with aabb solid angle and sggx cross-section
// 2-> after 12/03/26: completely different function:
//                          spatial: 3D gaussian per node, sampled a few times to evaluate several xp_i points
//                          angular: VMF fit per node. We also have the LUT to evaluate the integral of a given theta1, theta2, deltaphi, kappa 
//                          diffusee albedo: mean diffuse albedo (Everything weighted by area)
//                          importance = (1/N) * sum_i evalLength2Contribution(xs, xp_i, xe) * LUT(theta_xs_i, theta_xe_i, phi_i, kappa_node) * meanAlbedo
    ...
    const float cosinePenaltyScale)
{
    ...
    Float baseImportance = nodeInfo.s_diffuse_albedo.getLuminance();
...
#elif NODE_IMPORTANCE_VERSION == 1
...
Float computeNodeImportance(
    ...
    //     return .00001f*M_2_PI * nodeInfo.s_diffuse_albedo.getLuminance();
    // }
    ...
    Vector dirToNode = representativePoint - its_xs.p;
    ...
    Vector dirFromNode = pRec_xe.p - representativePoint;
    ...
    Float cosTheta_xs = dot(Vector(its_xs.shFrame.n), -wIn); // direction xs -> node
    if (cosTheta_xs <= 0.0f)
    ...
    // It naturally handles "backfacing" for volumetric clusters better than a single mean normal.
        ...
        // SGGX cross section term: favors nodes that are "facing" the right way
    ...
    #else
    Float sggxIn = 1.f, sggxOut = 1.f; // TODO: VMF HERE
    #endif
    ...
    Float omega_xs = AABBSphSolidAngle(nodeBounds, its_xs.p);
    Float omega_xe = AABBSphSolidAngle(nodeBounds, pRec_xe.p);
    ...
    Float baseImportance = nodeInfo.getMeanDiffuseAlbedoLum();
...
#elif NODE_IMPORTANCE_VERSION == 2
// Hopefully last iteration: 3D Gaussian, VMF, etc.
...
Float computeNodeImportance(
    ...
    const char *branch = "unscented_lut";
    ...
    if (bvh->m_debug2 > 0.5f) {
        branch = "unscented_lut_specular";
        importance = computeNodeImportance_Unscented_LUT_Specular(scene, bvh, nodeIndex, its_xs, pRec_xe);
#ifdef DEBUG_BVH_NAN
        if (impNonFinite(importance)) {
            impLogValue(branch, "return.importance", importance, bvh, nodeIndex, its_xs, pRec_xe);
        }
#endif
        return importance;
    }
    ...
    if (bvh->m_debug3 < 0.5f) {
        branch = "unscented_lut_original";
        importance = computeNodeImportance_Unscented_LUT_original(scene, bvh, nodeIndex, its_xs, pRec_xe);
#ifdef DEBUG_BVH_NAN
        if (impNonFinite(importance)) {
            impLogValue(branch, "return.importance", importance, bvh, nodeIndex, its_xs, pRec_xe);
        }
    ...
    if (bvh->m_debug3 >= 1.5f) {
        branch = "unscented_lut_fast";
        importance = computeNodeImportance_Unscented_LUT_fast(scene, bvh, nodeIndex, its_xs, pRec_xe);
#ifdef DEBUG_BVH_NAN
        if (impNonFinite(importance)) {
            impLogValue(branch, "return.importance", importance, bvh, nodeIndex, its_xs, pRec_xe);
        }
    ...
    importance = computeNodeImportance_Unscented_LUT(scene, bvh, nodeIndex, its_xs, pRec_xe);
#ifdef DEBUG_BVH_NAN
    if (impNonFinite(importance)) {
        impLogValue(branch, "return.importance", importance, bvh, nodeIndex, its_xs, pRec_xe);
    }
...
Float computeNodeImportance_Unscented_LUT_fast(
    ...
    const char *debugStage = "unscented_lut_fast";
...
#ifdef DEBUG_BVH_NAN
    if (impNonFinite(kappa)) {
        impLogValue(debugStage, "kappa", kappa, bvh, nodeIndex, its_xs, pRec_xe);
    }
    if (impNonFinite(covTrace)) {
        impLogValue(debugStage, "covTrace", covTrace, bvh, nodeIndex, its_xs, pRec_xe);
    }
#endif
...
#ifdef DEBUG_BVH_NAN
    if (impNonFiniteF(vmf_convolution)) {
        impLogValueF(debugStage, "vmf_convolution", vmf_convolution, bvh, nodeIndex, its_xs, pRec_xe);
        std::cerr << "[DEBUG_BVH_NAN] unscented_lut_fast vmf inputs"
                  << " w1_local=" << w1_local.toString()
                  << " w2_local=" << w2_local.toString()
                  << " d_sp_mean=" << d_sp_mean.toString()
                  << " d_pe_mean=" << d_pe_mean.toString()
                  << std::endl;
    }
    if (!f_xs.isValid() || !f_xp.isValid() || !Le.isValid() || !base_contribution.isValid() || impNonFinite(base_lum)) {
        std::cerr << "[DEBUG_BVH_NAN] unscented_lut_fast non-finite directional term"
                  << " f_xs=" << f_xs.toString()
                  << " f_xp=" << f_xp.toString()
                  << " Le=" << Le.toString()
                  << " base_contribution=" << base_contribution.toString()
                  << " base_lum=" << base_lum
                  << std::endl;
        impLogNodeContext(debugStage, bvh, nodeIndex, its_xs, pRec_xe);
    }
#endif
    ...
    #ifdef DEBUG_BVH_NAN
        if (impNonFinite(G_total) || impNonFinite(lum)) {
            std::cerr << "[DEBUG_BVH_NAN] unscented_lut_fast non-finite per-point"
                  << " pointIdx=" << pointIdx
                  << " pointWeight=" << pointWeight
                  << " xp=" << xp.toString()
                  << " dist_sp_sq=" << dist_sp_sq
                  << " dist_pe_sq=" << dist_pe_sq
                  << " G_total=" << G_total
                  << " base_lum=" << base_lum
                  << " lum=" << lum
                  << " d_sp=" << d_sp.toString()
    ...
    Float importance = nodeInfo.surfaceArea * weightedLuminance;
#ifdef DEBUG_BVH_NAN
    if (impNonFinite(weightedLuminance) || impNonFinite(importance)) {
        std::cerr << "[DEBUG_BVH_NAN] unscented_lut_fast non-finite final"
                  << " weightedLuminance=" << weightedLuminance
                  << " surfaceArea=" << nodeInfo.surfaceArea
...
Float computeNodeImportance_Unscented_LUT_original(
    ...
    const char *debugStage = "unscented_lut_original";
    ...
    for (size_t pointIdx = 0; pointIdx < numPointsToEval; ++pointIdx) {
        ...
        if (impNonFiniteF(vmf_convolution)) {
            impLogValueF(debugStage, "vmf_convolution", vmf_convolution, bvh, nodeIndex, its_xs, pRec_xe);
            std::cerr << "[DEBUG_BVH_NAN] unscented_lut_original vmf inputs"
                  << " pointIdx=" << pointIdx
                  << " xp=" << xp.toString()
                  << " w1_local=" << w1_local.toString()
                  << " w2_local=" << w2_local.toString()
    ...
    #ifdef DEBUG_BVH_NAN
        if (!contribution.isValid() || impNonFinite(G_sp) || impNonFinite(G_pe)) {
            std::cerr << "[DEBUG_BVH_NAN] unscented_lut_original non-finite contribution"
                  << " pointIdx=" << pointIdx
                  << " xp=" << xp.toString()
                  << " dist_sp=" << dist_sp
                  << " dist_pe=" << dist_pe
...
#ifdef DEBUG_BVH_NAN
    if (impNonFinite(weightedLuminance) || impNonFinite(importance)) {
        std::cerr << "[DEBUG_BVH_NAN] unscented_lut_original non-finite final"
                  << " weightedLuminance=" << weightedLuminance
                  << " surfaceArea=" << nodeInfo.surfaceArea
                  << " importance=" << importance
                  << std::endl;
...
MTS_EXPORT_RENDER Float computeNodeImportance_Unscented_LUT(
    ...
    const char *debugStage = "unscented_lut";
    ...
    // --- Optimization 1: Solid Angle (Perspective Spread) Check ---
    Float dist_xs_sq = (mean - its_xs.p).lengthSquared();
    Float dist_xe_sq = (mean - pRec_xe.p).lengthSquared();
    
    // Use the shortest distance to ensure it's clustered from BOTH perspectives
    Float min_dist_sq = std::min(dist_xs_sq, dist_xe_sq);
    ...
    // covTrace roughly bounds the squared radius. 
    // covTrace / min_dist_sq is our proxy for the maximum solid angle.
    // A threshold of 1e-4 to 1e-3 is usually a safe starting point.
    // const Float solidAngleThreshold = M_PI * 1e-2; 
    ...
    if (isSpatiallyClustered) {
        // Bypass Cholesky and point generation entirely
        sigmaPoints[0] = { mean, 1.0f };
    } else {
        sigmaPoints = precomputedSigmaPoints;
        numPointsToEval = 7;
    ...
    for (size_t pointIdx = 0; pointIdx < numPointsToEval; ++pointIdx) {
        ...
        if (impNonFiniteF(vmf_convolution)) {
            impLogValueF(debugStage, "vmf_convolution", vmf_convolution, bvh, nodeIndex, its_xs, pRec_xe);
            std::cerr << "[DEBUG_BVH_NAN] unscented_lut vmf inputs"
                  << " pointIdx=" << pointIdx
                  << " xp=" << xp.toString()
                  << " w1_local=" << w1_local.toString()
                  << " w2_local=" << w2_local.toString()
    ...
    #ifdef DEBUG_BVH_NAN
        if (!contribution.isValid() || impNonFinite(G_sp) || impNonFinite(G_pe)) {
            std::cerr << "[DEBUG_BVH_NAN] unscented_lut non-finite contribution"
                  << " pointIdx=" << pointIdx
                  << " xp=" << xp.toString()
                  << " dist_sp=" << dist_sp
                  << " dist_pe=" << dist_pe
...
#ifdef DEBUG_BVH_NAN
    if (impNonFinite(weightedLuminance) || impNonFinite(importance)) {
        std::cerr << "[DEBUG_BVH_NAN] unscented_lut non-finite final"
                  << " weightedLuminance=" << weightedLuminance
                  << " surfaceArea=" << nodeInfo.surfaceArea
                  << " importance=" << importance
                  << std::endl;
...
MTS_EXPORT_RENDER Float computeNodeImportance_Unscented_LUT_Specular(
    ...
    const char *debugStage = "unscented_lut_specular";
    if (!scene || !bvh || !bvh->isBuilt())
        return 0.0f;
    ...
    for (size_t pointIdx = 0; pointIdx < numPointsToEval; ++pointIdx) {
...
#ifdef DEBUG_BVH_NAN
        if (impNonFiniteF(vmf_diffuse) || impNonFiniteF(vmf_specular)) {
            if (impNonFiniteF(vmf_diffuse)) {
                impLogValueF(debugStage, "vmf_diffuse", vmf_diffuse, bvh, nodeIndex, its_xs, pRec_xe);
            }
            if (impNonFiniteF(vmf_specular)) {
                impLogValueF(debugStage, "vmf_specular", vmf_specular, bvh, nodeIndex, its_xs, pRec_xe);
            }
            std::cerr << "[DEBUG_BVH_NAN] unscented_lut_specular vmf inputs"
                      << " pointIdx=" << pointIdx
                      << " xp=" << xp.toString()
                      << " w1_local=" << w1_local.toString()
                      << " w2_local=" << w2_local.toString()
        ...
        Spectrum f_xp = nodeInfo.getMeanDiffuseAlbedo() * (1.0f / M_PI) * vmf_diffuse
    ...
    #ifdef DEBUG_BVH_NAN
        if (!contribution.isValid() || impNonFinite(G_sp) || impNonFinite(G_pe)) {
            std::cerr << "[DEBUG_BVH_NAN] unscented_lut_specular non-finite contribution"
                  << " pointIdx=" << pointIdx
                  << " xp=" << xp.toString()
                  << " dist_sp=" << dist_sp
                  << " dist_pe=" << dist_pe
...
#ifdef DEBUG_BVH_NAN
    if (impNonFinite(weightedLuminance) || impNonFinite(importance)) {
        std::cerr << "[DEBUG_BVH_NAN] unscented_lut_specular non-finite final"
                  << " weightedLuminance=" << weightedLuminance
                  << " surfaceArea=" << nodeInfo.surfaceArea
                  << " importance=" << importance
                  << std::endl;
